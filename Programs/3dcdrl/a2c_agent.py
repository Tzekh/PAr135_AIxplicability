...
class A2CAgent():
    ...
    def evaluate(self, test_envs, step, train_iters, num_games=128, movie_dir=''):
        ...
        mean_rewards = accumulated_rewards / num_games
        game_times = step
    ...
    def save_policy2(self, total_num_steps, args, output_dir):
        checkpoint = {'args': args,
                      'model': self.policy.state_dict(),
                      'optimizer': self.optimizer.state_dict()}

        filepath = os.path.join(output_dir, 'models/')
        torch.save(checkpoint, '{}checkpoint_{:001}.pth.tar'.format(filepath, total_num_steps))

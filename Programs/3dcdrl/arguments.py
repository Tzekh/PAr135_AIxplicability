...
def parse_a2c_args():
    ...
    paa('--scenario',           default='custom_scenario_test{:003}.cfg',    help='The scenario',  type=str)
    ...
    paa('--limit_actions',      default=True,                     help='limited the size of the action space to F, L, R, F+L, F+R', action='store_true')
    paa('--scenario_dir',       default='scenarios_transfer_learning/mazes_classic_test/',           help='location of scenarios',  type=str)
    ...
    paa('--multimaze',          default=True,                     help='Are there multiple maze environments',  action='store_true')
    paa('--num_mazes_train',    default=256,                        help='the number of training mazes, only valid for multimaze', type=int )
    paa('--num_mazes_test',     default=64,                        help='the number of testing mazes, only valid for multimaze', type=int)
    ...
    paa('--reload_model',       default='Transfer_learning/,0199886848',     help='directory and iter of model to load dir,iter', type=str) #
    paa('--model_checkpoint',   default='final.pth.tar',     help='the name of a specific model to evaluate, used when making videos', type=str)
    ...
    #               Training Parameters 
    # =========================================================================    
    paa('--learning_rate',      default=7e-5,       help='training learning rate', type=float)
    ...
    paa('--max_iters',          default=10000,    help='maximum number of training iterations', type=int) # initialement : 5000000
    paa('--eval_freq',          default=800,       help='how often the model is evaluated, in games', type=int)
    paa('--eval_games',         default=50,         help='the number of games the model is evaluated over', type=int)
    paa('--model_save_rate',    default=100,       help='How often to save the model in iters', type=int)
    ...
    paa('--num_frames',         default=20000000,  help='total number of frames', type=int)    #INITIALEMENT 200000000
    ...
    paa('--out_dir',        default='/home/adam/Bureau/3d_control_deep_rl-master/3dcdrl/',                  help='output directory for log files etc', type=str,)
